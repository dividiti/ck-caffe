...
typedef std::pair<float, int_tp> Prediction;
...
/* Return the top N predictions. */
  std::vector<Prediction> Classify(const cv::Mat& img, int_tp N) {
	  std::vector<float> output = Predict(img);
	  
	  std::vector<Prediction> pairs;
	  for (size_t i = 0; i < output.size(); ++i)
		pairs.push_back(std::make_pair(output[i], static_cast<int_tp>(i)));
...
// Set device id and mode
  static void set_device_id_and_mode() {
	  vector<int> gpus;
	  get_gpus(&gpus);
	  if (gpus.size() != 0) {
	#ifndef CPU_ONLY
